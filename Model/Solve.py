...
from SegDataFolder import semData, getTransform
...
from modeling.deeplab import *  #试试跑deeplab的时候注释掉这段可不可以！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！

#from models.networks.TransUnet import *   #三选一
#from models.networks9.TransUnet import *
#from models.networks_GN.TransUnet import *
#from models.networks_atten.TransUnet import *
...
class Solver(object):
    def __init__(self,configs):
        ...
        self.train_dataloader = torch.utils.data.DataLoader(self.trainSet, batch_size=self.batchsize, shuffle=True, num_workers=4)   #shuffe 打乱顺序
        self.val_dataloader = torch.utils.data.DataLoader(self.valSet, 1, shuffle=False, num_workers=4)
        self.best_miou = 0.00
        self.result_dir = self.configs['result_dir']
        os.makedirs(self.result_dir,exist_ok=True) #只有在目录不存在时创建目录，目录已存在时不会抛出异常,False表示文件存在使报错。
...
#  修改程序运行状态，使其被暂停后仍可接着跑
    def resume_state(self, path):
        checkpoint = torch.load(path)  #加载模型参数， torch.save(net.state_dict(),path)保存模型参数
...
#  logInfo生成日志信息表，“:.4f”为保留小数点后四位
            if (i+1)%self.logIterval == 0:
                logInfo = ("Train [{}/{}]\t"
                          "Loss val {loss_meter.val:.4f}(avg {loss_meter.avg:.4f})\t"
                          "Accuracy {acc:.4f}\t"
...
#  计算每一次迭代的allacc/macc/miou
        ...
        loss1 = loss_meter.avg

#  logInfo生成日志信息表，“:.4f”为保留小数点后四位
        logInfo = 'Train at epoch [{}/{}] : mIoU/mAcc/allAcc/loss1 {:.4f}/{:.4f}/{:.4f}/{:.4f}.'.format(epoch+1, self.end_epoch, mIoU, mAcc, allAcc, loss1)
        mIoU1 = ('{:.4f}'.format(mIoU))
        mAcc1 = ('{:.4f}'.format(mAcc))
        allAcc1 = ('{:.4f}'.format(allAcc))
        Loss1 = ('{:.4f}'.format(loss1))
        file_hand = open('train.txt', mode="a+")
        file_hand.write('mIOU:' + mIoU1 + ', ' + 'mAcc:' + mAcc1 + ', ' + 'allAcc:' + allAcc1 + ', ' + 'Loss:' + Loss1 + '\n')
        file_hand.close()
        print(logInfo)
...
#  计算每一次迭代间隔的loss/acc/iou
        ...
        #    f1 = f1_score(correct, preds)                 # F1分数可以看作是模型精确率和召回率的一种调和平均
            ...
            if mIoU > self.best_miou:
                self.best_miou = mIoU
                self.save_state(epoch, '{}/{}-{:.4f}-ep{}.pth'.format(self.result_dir,self.configs['net'], mIoU, epoch+1))
                self.save_pred()

          # logInfo = 'Val at epoch [{}/{}] : mIoU/mAcc/allAcc {:.4f}/{:.4f}/{:.4f}.\n'.format(epoch+1, self.end_epoch, mIoU, mAcc, allAcc)
            logInfo = 'Val at epoch [{}/{}] : mIoU/mAcc/allAcc {:.4f}/{:.4f}/{:.4f}/{:.4f}.'.format(epoch + 1, self.end_epoch, mIoU, mAcc, allAcc, loss2)
          # logInfo += '1-precision : {:.4f} | 1-recall : {:.4f} | f1 : {:.4f}.'.format(1-precision, 1-recall, f1)
            ...
            Loss2 = ('{:.4f}'.format(loss2))
            file_hand = open('test.txt', mode="a+")
            file_hand.write(mIoU2 + ',' + mAcc2 + ',' + allAcc2 + ',' + Loss2 + '\n')
            file_hand.close()
            print(logInfo)
    ...
    def test(self, prefix):
        ...
        with torch.no_grad():
            for n, img in self.trainSet.TestSetLoader():
                ...
                outp = decode_segmap(outp)  #将分割图转化为可视化图像，更容易查看分割效果
    ...
    def trainer(self):
        try:
            ...
            for epoch in range(self.start_epoch, self.end_epoch):
                self.train(epoch)
                self.scheduler.step()
                self.save_state(epoch, '{}/{}-ep{}.pth'.format(self.result_dir,self.configs['net'], epoch+1))
...
if __name__ == "__main__":
    ...
    time_end = time.time()
    stoptime = datetime.datetime.now().strftime('%x-%X')
    print('开始时间：{}，结束时间：{}，spend_time:{}'.format(starttime, stoptime, time_end - time_start))